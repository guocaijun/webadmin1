...
//商家食品列表
app.get("/admingood", (req, res) => {
    //获取数据
    let sql = "select *,goods.id as gid,shoplists.id as sid from goods,shoplists where goods.shoplist_id=shoplists.id"
    //执行sql
    connection.query(sql, (error, results, fields) => {
        if (error) {
            throw error;
        ...
        // return;
        // res.end();
        res.render("AdminGood/admingood", {
            data: results,
            loginnames: req.session.usernames
        });
        res.end();
...
//入驻商家的添加
app.get("/admingoodadd", checklogin, (req, res) => {
    //获取商家数据
    let sql="select * from shoplists";
    connection.query(sql, (error, results, fields) => {
        if (error) {
        ...
        // return;
        // res.end();
        res.render("AdminGood/admingoodadd", {
            data: results,
            loginnames: req.session.usernames
        });
        res.end();
...
//入驻商家食品执行添加
app.post("/admingooddoadd", checklogin, (req, res) => {
    //实例化formidable
    let form = new formidable.IncomingForm();
    //规定上传文件路径
    form.uploadDir = "./uploads"
    ...
    form.parse(req, (err, fields, files) => {
        ...
        // res.end();
        //准备sql语句
        let sql = "insert into goods(foodname,foodpic,descr,price,shoplist_id)values('" + foodname + "','" + foodpic + "','" + descr + "','" + price + "',"+shoplist_id+")";
        // //执行sql
        connection.query(sql, (error, results, fields) => {
            if (error) {
                throw error;
...
//食品执行删除
app.get("/admingooddel", checklogin, (req, res) => {
    // res.write("this is delete");
    //获取需要删除数据的id
    let id = req.query.id;
    //sql
